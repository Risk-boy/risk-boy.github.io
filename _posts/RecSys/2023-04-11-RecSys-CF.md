...
- 아이템 $i$에 대한 평점 예측을 하기 위해서는 $\ohm_i$에 속한 모든 유저와의 유사도를 구해야함
  - $\ohm_i:$ 아이템 $i$에 대해 평가를 한 유저 집합
...
$\ohm_i$에 속한 유저 가운데 유저 $u$와 가장 유사한 $K$명의 유저(KNN)를 이용해 평점을 예측
...
$$
\hat{r}(u,i)=\frac{\sum_{u^{'}\in \ohm_i}sim(u, u^{'})r(u^{'},i)}{\sum_{u^{'}\in \ohm_{i}}sim(u,u^{'})}
$$
**Absolute Rating의 한계**
...
$$
\widehat{dev}(u,i)=\frac{\sum_{u^{'}\in \ohm_i}dev(u^{'},i)}{|\ohm_i|}=\frac{\sum_{u^{'}\in \ohm_i}r(u^{'},i)-\overline{r_{u^{'}}}}{|\ohm_i|}
$$

$$
\hat{r}(u,i)=\bar{r}_u+\frac{\sum_{u^{i}\in \ohm_i}r(u^{'},i)-\overline{r_{u^{'}}}}{|\ohm_i|}=\overline{r_u}+\widehat{dev}(u,i)
$$

$$
\hat{r}(u,i)=\overline{r_u}+\frac{\sum_{u^{'}\in \ohm_i}sim(u, u^{'})\{r(u^{'},i)-\overline{r_{u^{'}}}\}}{\sum_{u^{'}\in \ohm_{i}}sim(u,u^{'})}
$$
